...
void daemonize(void) {
	...
	int res = chdir("/");
    if (res != 0) {
        logmsg(0, LOG_ERR, "Cannot change chdir to /. err: %d", res);
    }
...
static void pbl_set_verdict(struct PBL_HANDLE *h, PBL_ID_T id,
	...
	if (conf.dryrun == 1) {
		...
		//SET_VERDICT(h, id, NF_DROP, 0, NULL);
...
int main(int argc, char **argv) {
	...
	{
		if (getcwd(work_directory, sizeof(work_directory)) == NULL) {
            logmsg(0, LOG_ERR, "Failed to get current directory.");
        }
...
*/
	...
	while (1) {
        ...
        //snprintf(lookupbuf, sizeof(lookupbuf), "%hhu.%hhu.%hhu.%hhu.%s", ip->b4, ip->b3, ip->b2, ip->b1,
			//wltmp->string);
		snprintf(lookupbuf, sizeof(lookupbuf), "%hhu.%hhu.%hhu.%hhu", ip->b1, ip->b2, ip->b3, ip->b4);


		if(false && regexec(&regex, wltmp->string, (size_t) 0, NULL, 0) != REG_NOMATCH ) 
		{
...
//				ldns_resolver_deep_free(res_def);
		...
		else
		{
			...
			logmsg(-1, LOG_DEBUG, "Sending to default macrometa %s", lookupbuf);
//#ifndef HAVE_FIREDNS
...
//				;
            if (!is_blacklisted_mm(lookupbuf, conf.macrometa_app_key, conf.macrometa_endpoint, ipblc_id)) {
				...
				// found.
				logmsg(-1, LOG_NOTICE, "The IP %s was found",	lookupbuf);
				if (conf.debug > 1)
				{
					end_req = cur_time();
					logmsg(1, LOG_DEBUG, "finish tread, thread time (%.3f sec)",	(end_req - start_req) / 1000000.0);
